...
class TerminalReporter:
    ...
    def report_trace(self, trace: Trace, show_io: bool = False) -> None:
        ...
        self.console.print(Panel(
            ...
            f"[dim]{trace.duration_ms:.0f}ms"
            f" · {trace.span_count} spans · {trace.total_tokens} tokens[/]",
            expand=False,
        ))
        self._print_span_tree(trace, show_io)

    def _print_span_tree(
        self, trace: Trace, show_io: bool, parent_id: str | None = None, depth: int = 0
    ) -> None:
        ...
        for span in spans:
            ...
            if show_io and span.inputs:
                self.console.print(
                    f"{'  ' * (depth + 2)}[dim]in:[/] {str(span.inputs)[:100]}"
                )
            if show_io and span.outputs:
                self.console.print(
                    f"{'  ' * (depth + 2)}[dim]out:[/] {str(span.outputs)[:100]}"
                )
